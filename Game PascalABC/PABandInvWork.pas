...
program PABandInvWork;

uses graphABC, ABCObjects;

var 
  x1, y1, sum, Frame: integer;
  scene1: string;
...
var 
  ...
  xi, yi, xi1, yi1, xm1, ym1, xt, yt, xi2, yi2: integer;
...
begin
  if (objButtonInv.PtInside(xm, ym)= true) then begin scene:= 'inventar'; frame:=0; end;
end;
...
begin
  ...
  scene:='PanelAndButtoms';
  while 1=1 do begin
    if (scene='PanelAndButtoms')and(frame<>1) then begin frame:=1; ImportantPartProgramPanelAndButtoms(scene); end;
    if (scene='inventar')and(frame<>1) then begin frame:=1; ImportantPartProgramInv(scene); end;
  end;
end.